...
class AuthStore:
    ...
    def __init__(self, service_name: str = "kiwi-mcp"):
        ...
        Args:
            service_name: Service name for keyring storage (default: "kiwi-mcp")
        """
    ...
    async def get_token(self, service: str, scope: Optional[str] = None) -> str:
            ...
            if expires_at > datetime.now(timezone.utc):
                ...
                if scope is None or scope in cached.get("scopes", []):
                    ...
                    raise AuthenticationRequired(
                        ...
                        f"Please sign in again with: kiwi auth signin"
                    )
        ...
        if not token:
            raise AuthenticationRequired(
                f"No authentication token for {service}. Please sign in with: kiwi auth signin"
            )
