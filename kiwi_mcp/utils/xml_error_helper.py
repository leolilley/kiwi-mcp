...
def extract_context_lines(
    xml_content: str, line_num: int, context_lines: int = 2
) -> Dict[str, any]:
    """
...
def detect_problematic_characters(line: str, column: Optional[int] = None) -> List[Dict[str, any]]:
    ...
    # Check for shell operators that suggest CDATA is needed
    shell_operators = ["&&", "||", "|", ">>", ">", "<", "2>&1", "$", "`"]
    for op in shell_operators:
        if op in line:
            # Only add once
            if not any(i["pattern"] == "shell_operator" for i in issues):
                issues.append(
                    {
                        "character": op,
                        "position": line.find(op),
                        "pattern": "shell_operator",
                        "suggestion": "Wrap in CDATA: <command><![CDATA[shell_command]]></command>",
                    }
                )
            break
...
def _generate_suggestion(char: str, pattern_type: str) -> str:
    ...
    if pattern_type == "comparison_operator":
        ...
        # For shell operators, suggest CDATA instead of escaping
        if "&&" in char or "||" in char or char == "&":
            return "Wrap shell commands in CDATA: <command><![CDATA[content]]></command>"
        return char.replace("&", "&amp;")
...
def format_error_with_context(
    error_msg: str, xml_content: str, file_path: Optional[str] = None
) -> str:
    """
    ...
    if issues:
        ...
        for issue in issues[:3]:  # Limit to first 3 issues
            ...
            if pattern == "comparison_operator":
                ...
                parts.append(f"  - Found unescaped '&' → use CDATA for shell commands")
            elif pattern == "shell_operator":
                parts.append(f"  - Found shell operator → Wrap in CDATA for shell commands")
            else:
                parts.append(
                    f"  - Found '{char}' at position {issue['position']} → use '{suggestion}'"
                )
    ...
    parts.append("  1. Wrap shell commands in CDATA (RECOMMENDED):")
    parts.append("     <command><![CDATA[")
    parts.append("     ls -la && test -f file.txt")
    parts.append("     ]]></command>")
    parts.append("  2. Escape special characters: < → &lt;, > → &gt;, & → &amp;")
