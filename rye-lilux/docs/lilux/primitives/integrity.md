**Source:** Original implementation: `kiwi_mcp/primitives/integrity.py` in kiwi-mcp
...
def short_hash(full_hash: str, length: int = 12) -> str:
    """Return first N characters of hash for display."""
```
...
```python
full_hash = "abc123def456abc123def456abc123def456abc123def456abc123def456abcd"
short = short_hash(full_hash, 12)  # "abc123def456" (first 12 chars)

print(f"Tool abc123def456... v1.0.0")  # Nice display
```
...
# Later, verify
if compute_tool_integrity(...) == entry.integrity:
    print("Tool verified")
```
...
```python
import pytest
from lilux.primitives import compute_tool_integrity, short_hash
...
def test_short_hash():
    """Short hash returns prefix."""
    full = "abc123def456" * 5  # 60 chars
    short = short_hash(full, 12)
