...
RYE's universal executor uses EnvResolver when:
...
See `[[rye/categories/runtimes]]` for ENV_CONFIG schema.
...
### Runtime Definition (in RYE)

```yaml
# rye/.ai/tools/rye/runtimes/python.yml
name: "Python Runtime"
version: "1.0.0"
...
```python
# 1. RYE loads runtime config
runtime = get_runtime("python")
env_config = runtime.env_config

# 2. EnvResolver resolves environment
...
# 3. Execute subprocess with resolved environment
result = await subprocess.execute(
    config={
...
# If .venv not found, uses fallback
# env["INTERPRETER"] = "/usr/bin/python3"
```
...
# If .nonexistent not found and no fallback, var not set
# "INTERPRETER" key may be missing from env
```
