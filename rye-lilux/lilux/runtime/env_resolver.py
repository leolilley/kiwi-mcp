...
class EnvResolver:
    ...
    def __init__(self, project_path: Optional[Path] = None):
        ...
        from lilux.utils.path_service import get_user_space
    ...
    def _resolve_venv_python(self, config: Dict[str, Any]) -> Optional[str]:
        ...
        - lilux: {project}/.ai/scripts/.venv/bin/python
        - user: ~/.ai/.venv/bin/python
        - system: which python3 || which python
        """
        search = config.get("search", ["lilux", "user", "system"])
    ...
    def _resolve_node_modules(self, config: Dict[str, Any]) -> Optional[str]:
        ...
        - lilux: {project}/.ai/scripts/node_modules/.bin/node
    ...
    def _build_python_locations(self) -> Dict[str, Any]:
        ...
        Returns dict with keys: project, lilux, user, system
    ...
    def _load_dotenv_files(self) -> Dict[str, str]:
        ...
        if self.project_path:
            # Check both .ai/.env (lilux standard) and root .env files
            env_paths = [
                self.project_path / ".ai" / ".env",  # Lilux standard
