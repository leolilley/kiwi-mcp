...
class SimpleVectorStore(VectorStore):
    ...
    def __init__(
        self,
        storage_path: Path,
        collection_name: str = "lilux_items",
        ...
        self.source = source  # 'project' or 'user'
    ...
    async def search(
        ...
        for i, cand in enumerate(candidates):
            # Extract source from metadata if available, otherwise use store's source
            metadata_source = cand["metadata"].get("source", self.source)
            results.append(
                SearchResult(
    ...
    async def update(self, item_id: str, content: str, metadata: dict) -> bool:
        ...
        conn = self._get_conn()
        try:
            cursor = conn.execute("SELECT item_type FROM embeddings WHERE item_id = ?", (item_id,))
            row = cursor.fetchone()
        finally:
            conn.close()
    ...
    async def exists(self, item_id: str) -> bool:
        ...
        conn = self._get_conn()
        try:
            cursor = conn.execute("SELECT 1 FROM embeddings WHERE item_id = ?", (item_id,))
