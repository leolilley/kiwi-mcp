"""
Logger utility for lilux.

Implements rotating file logs in user space ($USER_SPACE/logs/):
- lilux.log: Main log with 5MB rotation, keeps 3 backups
- lilux.errors.log: Errors only, 2MB rotation, keeps 2 backups
- lilux.json: Structured JSON, 5MB rotation, keeps 2 backups
...
def get_logger(name: str, level: Optional[int] = None) -> logging.Logger:
    ...
    - $USER_SPACE/logs/lilux.log (rotating, 5MB max, 3 backups)
    - $USER_SPACE/logs/lilux.errors.log (errors only, 2MB max, 2 backups)
    - $USER_SPACE/logs/lilux.json (structured JSON, 5MB max, 2 backups)
    ...
    if not logger.handlers:
        ...
        try:
            ...
            error_log = log_dir / "lilux.errors.log"
...
def cleanup_old_logs(days: int = 30) -> int:
    ...
    try:
        ...
        for log_file in log_dir.glob("lilux*.log*"):
