...
Single source of truth for all path resolution in lilux.
...
def _get_platform() -> str:
    """Get platform identifier: 'linux', 'darwin', or 'win32'."""
    return sys.platform
...
def get_rye_state_dir() -> Path:
    """
    Get RYE state directory for persistent runtime state.
    
    NOT for content - this is for signing keys, sessions, telemetry.
    Uses XDG_STATE_HOME on Linux, platform-specific paths elsewhere.
    ...
    if platform == "win32":
        ...
        if local_app_data:
            return Path(local_app_data) / "rye"
        return Path.home() / "AppData" / "Local" / "rye"
    
    elif platform == "darwin":
        # macOS: ~/Library/Application Support/rye/
        return Path.home() / "Library" / "Application Support" / "rye"
    
    else:
        # Linux/BSD: $XDG_STATE_HOME/rye/ → ~/.local/state/rye/
        xdg_state = os.getenv("XDG_STATE_HOME")
        if xdg_state:
            return Path(xdg_state) / "rye"
        return Path.home() / ".local" / "state" / "rye"


def get_lilux_cache_dir() -> Path:
    """
    Get Lilux cache directory for ephemeral data.
    
    For HTTP cache, vector embeddings, etc. Re-downloadable/rebuildable.
    Uses XDG_CACHE_HOME on Linux, platform-specific paths elsewhere.
    ...
    if platform == "win32":
        ...
        local_app_data = os.getenv("LOCALAPPDATA")
        if local_app_data:
            return Path(local_app_data) / "lilux" / "Cache"
        return Path.home() / "AppData" / "Local" / "lilux" / "Cache"
    
    elif platform == "darwin":
        # macOS: ~/Library/Caches/lilux/
        return Path.home() / "Library" / "Caches" / "lilux"
    
    else:
        # Linux/BSD: $XDG_CACHE_HOME/lilux/ → ~/.cache/lilux/
        xdg_cache = os.getenv("XDG_CACHE_HOME")
        if xdg_cache:
            return Path(xdg_cache) / "lilux"
        return Path.home() / ".cache" / "lilux"
