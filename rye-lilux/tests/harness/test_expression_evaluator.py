...
if not _tools_path.exists() or not (_tools_path / "expression_evaluator.py").exists():
    pytest.skip("No expression_evaluator (need rye/.ai or .ai/tools/threads)", allow_module_level=True)
_spec = importlib.util.spec_from_file_location("expression_evaluator", _tools_path / "expression_evaluator.py")
_expr_module = importlib.util.module_from_spec(_spec)
sys.modules["expression_evaluator"] = _expr_module
...
resolve_path = _expr_module.resolve_path
tokenize = _expr_module.tokenize
Parser = _expr_module.Parser
ExpressionError = _expr_module.ExpressionError
