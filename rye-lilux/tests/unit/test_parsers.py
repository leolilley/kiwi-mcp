...
"""
    ...
    def test_parse_directive_with_nested_template(self, tmp_path, parsing_project_path):
        ...
        # This simulates create_directive which has a template containing a full directive example
        directive_content = '''# Create Something

```xml
<directive name="create_something" version="1.0.0">
  <metadata>
    <description>Create with template</description>
    <category>test</category>
    <model tier="fast" />
  </metadata>
  <process>
...
'''
        ...
        assert "template" in result["templates"]
        # Template should contain the nested directive example
        assert "[NAME]" in result["templates"]["template"]
        assert "<directive" in result["templates"]["template"]
    ...
    def test_parse_directive_with_nested_markdown_fences(self, tmp_path, parsing_project_path):
        ...
        directive_content = '''# Example Directive

```xml
<directive name="example_fences" version="1.0.0">
  <metadata>
    <description>Has nested fences</description>
    <category>test</category>
    <model tier="fast" />
  </metadata>
  <process>
    <step name="show_example">
      <action>Here is an example</action>
      <example>
```python
def hello():
    print("world")
...
'''
        ...
        assert "templates" in result
        # The example tag is treated as opaque
        assert "example" in result["templates"]
        assert "def hello():" in result["templates"]["example"]
